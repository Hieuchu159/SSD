...
def train_model(net, dataloader_dict, criterion, optimizer, num_epochs):
    ...
    epoch_val_loss = 0.0
    logs = []
    loss_values = {"train": {"total": [], "loc": [], "conf": []}, "val": {"total": [], "loc": [], "conf": []}}
    
    with open('./data/loss_values.csv', 'w', newline='') as csvfile:
        fieldnames = ['epoch', 'iteration', 'phase', 'total_loss', 'loc_loss', 'conf_loss']
        ...
        for epoch in range(num_epochs+1):
            ...
            for phase in ["train", "val"]:
                ...
                for images, targets in dataloader_dict[phase]:
                    ...
                    with torch.set_grad_enabled(phase=="train"):
                        ...
                        loss_values[phase]["total"].append(loss.item())
                        loss_values[phase]["loc"].append(loss_l.item())
                        loss_values[phase]["conf"].append(loss_c.item())
            ...
            for phase in ["train", "val"]:
                for i, (total_loss, loc_loss, conf_loss) in enumerate(zip(loss_values[phase]["total"], loss_values[phase]["loc"], loss_values[phase]["conf"])):
                    writer.writerow({'epoch': epoch+1, 'iteration': i+1, 'phase': phase, 'total_loss': total_loss, 'loc_loss': loc_loss, 'conf_loss': conf_loss})
